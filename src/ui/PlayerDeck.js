...
const CARD_POSITIONS = {
  ...
  [WHEAT_CARD_KEY]: { x: 0, y: 320 },
  [SHEEP_CARD_KEY]: { x: 0, y: 480 },
...
export default class PlayerDeck {
  ...
  createResourceCard(scene, imageKey, numberOfCards) {
    let numberOfCardsForOffset = numberOfCards > MAX_CARDS_TO_SHOW ? MAX_CARDS_TO_SHOW : numberOfCards;
    let uiArray = [];
    ...
    if (numberOfCards <= MAX_CARDS_TO_SHOW) {
    ...
    let cardText = scene.add.text(cardPosition.x, cardPosition.y, numberOfCards, { fontSize: '32px' });
